...
export class Jobs extends Component {
  ...
  handleCreateClick = () => {
    ...
    const newJob = firebaseApp.database().ref(`/${path}`).push();

    newJob.update({ name: 'New Job(one word)' }).then(() => {
      history.push(`/${path}/edit/${newJob.key}/main`);
    })
...
export default connect(
  mapStateToProps
)(injectIntl(withFirebase(withRouter(Jobs))));
