...
export class User extends Component {
  ...
  handleAdminChange = (e, isInputChecked) => {
    ...
    if (isInputChecked) {
      ...
      firebaseApp.database().ref(`/user_roles/${uid}`).set({role: 'admin', company: 'all', active: true})
    } else {
      firebaseApp.database().ref(`/admins/${uid}`).remove()
      firebaseApp.database().ref(`/user_roles/${uid}`).set({role: 'customer', company: 'all', active: true})
    }
