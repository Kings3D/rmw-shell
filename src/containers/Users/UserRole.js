...
export class UserRole extends Component {
  ...
  componentDidMount() {
    const { watchList, watchPath, userRolePath } = this.props
  ...
  handleRoleToggleChange = (e, isInputChecked, key) => {
    const { firebaseApp, userRolePath } = this.props
    const ref = firebaseApp.database().ref(`${userRolePath}/${key}`)
...
const mapStateToProps = (state, ownProps) => {
  ...
  const userRolePath = rootPath ? `/${rootPath}_user_roles/${uid}/${rootUid}` : `/user_roles/${uid}`
  ...
  return {
    ...
    intl,
    userRolesPath,
    user_role: getPath(state, userRolePath),
